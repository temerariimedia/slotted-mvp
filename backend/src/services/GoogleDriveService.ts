...
export class GoogleDriveService {
  ...
  constructor(credentials: any) {
    ...
    try {
      ...
      throw new Error(`Failed to initialize Google Drive service: ${error instanceof Error ? error.message : 'Invalid credentials'}`);
    }
  }
  ...
  async createFolder(name: string, parentId?: string): Promise<DriveFolder> {
    try {
      ...
      throw new Error(`Failed to create folder "${name}": ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async createCampaignFolders(
    ...
    for (const folderName of subfolderNames) {
      ...
      const key = folderName.toLowerCase().replace(/[^a-z]/g, '').replace('rawassets', 'assets') as keyof CampaignFolderStructure['subfolders'];
