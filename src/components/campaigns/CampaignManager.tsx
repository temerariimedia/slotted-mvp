  ...
  const handleGenerateCampaigns = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL || 'http://localhost:3001'}/api/campaigns/generate-topics`,
        {
          method: 'POST',
          headers: {
      ...
      const generatedCampaigns: Campaign[] = data.topics.map((topic: any, index: number) => ({
        ...
        primaryChannel:
          topic.primaryChannel || context.marketingGoals?.channels?.primary?.[0] || 'Blog',
        secondaryChannels: topic.secondaryChannels ||
          context.marketingGoals?.channels?.secondary || ['Social Media'],
      ...
      alert(
        `Failed to generate campaigns: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    } finally {
  ...
  const handleCreateAssets = async () => {
    ...
    try {
      ...
      const calendarResponse = await fetch(
        `${process.env.REACT_APP_BACKEND_URL || 'http://localhost:3001'}/api/google/sheets/create-campaign-calendar`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      const folderResponse = await fetch(
        `${process.env.REACT_APP_BACKEND_URL || 'http://localhost:3001'}/api/google/drive/create-company-folder`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
