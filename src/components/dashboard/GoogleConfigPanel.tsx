  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL || 'http://localhost:3001'}/api/google/test-connection`,
        {
          method: 'POST',
          headers: {
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">{isConfigured ? 'Connected' : 'Not configured'}</p>
          </div>
          <button
          ...
          {testResults && (
            <div className="p-3 bg-gray-50 rounded-md">
              ...
              <div className="space-y-1 text-xs">
                <div className="flex items-center space-x-2">
                  <span
                    className={`w-2 h-2 rounded-full ${testResults.sheets ? 'bg-green-500' : 'bg-red-500'}`}
                  ></span>
                  <span>Google Sheets API: {testResults.sheets ? 'Working' : 'Failed'}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <span
                    className={`w-2 h-2 rounded-full ${testResults.drive ? 'bg-green-500' : 'bg-red-500'}`}
                  ></span>
