...
interface SlottedContextState {
  ...
  updateContext: (updates: Partial<SlottedContext>) => Promise<void>
...
export const SlottedContextProvider: React.FC<SlottedContextProviderProps> = ({ children }) => {
  ...
  const updateContext = async (updates: Partial<SlottedContext>) => {
    if (!context) return
  ...
  return <SlottedContextContext.Provider value={value}>{children}</SlottedContextContext.Provider>
}
